...
export class MusigService {
  ...
  aggregate(storedAccounts, runWithPubkeys = null) {
    ...
    if (runWithPubkeys && this.savedPublicKeys?.length > 1) {
      for (const pubKey of this.savedPublicKeys) {
        addresses.push(btcocurrency.deriveAddress(pubKey, {useBtcoPrefix: true}));
      }
    } else {
    ...
    const fullAddressFinal = 'btco_' + base32.encode(fullAddress);
  ...
  multiSign(privateKey, blockHash, inputMultisigData) {
    ...
    if (!this.musigStagePtr) {
      if (!this.util.btco.isValidHash(privateKey)) {
        throw new Error('Invalid private key');
      }
      if (!this.util.btco.isValidHash(blockHash)) {
      ...
      if (this.musigStageNum === 0) {
        ...
        const pub = btcocurrency.derivePublicKey(privateKey);
