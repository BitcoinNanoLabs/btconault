...
export class MultisigComponent implements OnInit {
  ...
  addAccount() {
    if (this.accountAddStatus !== 1) {
      ...
      this.notificationService.sendWarning('Invalid Bitcoin Nano address!', {identifier: 'account-invalid'});
      return;
    }
    if (this.storedAccounts.includes(this.accountAdd.replace('xrb_', 'btco_').toLocaleLowerCase())) {
    ...
    this.storedAccounts.push(this.accountAdd.replace('xrb_', 'btco_').toLocaleLowerCase());
    this.accountAdd = '';
  ...
  navigateAccount() {
    if (this.validateMultisig()) {
      ...
      this.notificationService.sendWarning('Invalid Bitcoin Nano account!');
    }
  }
  ...
  navigateBlock(block) {
    ...
    if (block.startsWith('btcosign:') || block.startsWith('btcoprocess:')) {
      const url = new URL(block);
      if (url.protocol === 'btcosign:') {
