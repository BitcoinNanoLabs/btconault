...
export class AddressBookComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async updateTrackedBalances(refresh= false) {
    ...
    for (const entry of this.addressBookService.addressBook) {
      ...
      // Add balances from RPC data
        ...
        balanceAccount.balanceFiat = this.util.btco.rawToMBtco(balanceAccount.balance).times(this.fiatPrice).toNumber();
        balanceAccount.balanceRaw = new BigNumber(balanceAccount.balance).mod(this.btco);
      }
  ...
  async saveNewAddress() {
    ...
    // Remove spaces and convert to btco prefix
    this.newAddressAccount = this.newAddressAccount.replace(/ /g, '').replace('xrb_', 'btco_');
  ...
  async exportAddressBook() {
    ...
    const exportUrl = `https://btconault.cc/import-address-book#${base64Data}`;
  ...
  exportAddressBookToFile() {
    const fileName = `BtcoNault-AddressBook.json`;
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (!importData.length || !importData[0].account) {
          return this.notificationService.sendError(`Bad import data, make sure you selected a BtcoNault Address Book export`);
        }
